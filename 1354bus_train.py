...
from tensorflow.keras.layers import Dense, Dropout, Conv1D, LSTM, PReLU, GRU, Bidirectional, ReLU , BatchNormalization
...
# nohup python 1354bus_split_train.py > output.log 2>&1 &
mode = 'train' # 'train' or 'data gen' or /'save_output' failed case일때 예측값 txt로 출력을 뽑을때  / 'save_excel' 엑셀 파일로 예측값 출력 저장할때
# test - 저장된 모델을 불러와서 test
nBus = 1354
nSamples = 1000
SysName = "1354Bus"
...
nPrd_list = [8] # ,24
GNN_types = ["DiffusionConv"] # "APPNP", "ECCConv","ARMA" , "DiffusionConv", "GATConv", "GCNConv", "GCSConv", "GINconvBatch"
    ...
    for gnn_type in GNN_types:
        ...
        for rnn_type in RNN_types:
            ...
            if mode == 'data gen':
                ...
                Bra_dataFile = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/case1354_edgedata.dat'
                Bra_data = pd.read_csv(Bra_dataFile)

                # fBus_ = Bra_data["branch_fbus"]
                # tBus_ = Bra_data["branch_tbus"]
                bus_mapping = np.load(f"{PROJECT_ROOT}/Data/1354bus_mapping.npy")
                nodeID_to_bus = {nodeID: new_bus for new_bus, nodeID in bus_mapping}
                Bra_data["branch_fbus"] = Bra_data["branch_fbus"].map(nodeID_to_bus)
                Bra_data["branch_tbus"] = Bra_data["branch_tbus"].map(nodeID_to_bus)

                fBus = Bra_data["branch_fbus"]
                tBus = Bra_data["branch_tbus"]

                kRating = Bra_data["branch_rateA"].copy()
                kRating[kRating == 0] = 15501.32
                kBranch_b = Bra_data["branch_b"]

                # #Read gen info
                # Gen_dataFile = '{PROJECT_ROOT}/Data/IEEE500Bus/DataCollectionOptimization(BaseSCUC)/GenData_1GPerBus_500Bus.dat'
                # Gen_data = pd.read_csv(Gen_dataFile)
                # # print (Gen_data.info)
                # gBus = Gen_data["genD_bus"]

                Demand_fileName = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/demand1354Bus_{nPrd}_all.txt'
                dfNDmd =  loadtxt(Demand_fileName, delimiter=',') # Nodal demand NBus*24-Hours sequences

                # 노드의 M A데이터셋
                madata = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/1354_{nPrd}_ma_values_all.txt'
                ...
                directory = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/AM'
                ...
                path = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/graph_data'
            ...
            if mode == 'train':
                ...
                # 경로 설정
                adjacency_matrix_path_sparse = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/AM/AM_sparse.npz'
                adjacency_matrix_path_dense = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/AM/AM_dense.npy'
                ...
                elif GNN == "GATConv" or GNN == "ECCConv" or GNN == "DNN":
                ...
                class GNNLSTMCmt(Model):
                    def __init__(self, **kwargs):
                        ...
                        # 엣지 정보 필요없는 layer
                            ...
                            # GATConv
                            head = 3
                            self.conv1 = GATConv(nPrd, attn_heads=head, concat_heads=True, dropout_rate=0.5, add_self_loops=True, activation=None, use_bias=False, kernel_initializer="glorot_uniform")
                            self.conv_middle = GATConv(nPrd, attn_heads=head, concat_heads=True, dropout_rate=0.5,add_self_loops=True, activation=None, use_bias=False, kernel_initializer="glorot_uniform")
                            self.conv2 = GATConv(nPrd, attn_heads=head, concat_heads=True, dropout_rate=0.5, add_self_loops=True, activation=PReLU(), use_bias=True, kernel_initializer="glorot_uniform")

                            # self.conv1 = GATConv(nPrd, attn_heads=head, concat_heads=True,
                            #                      add_self_loops=True, activation=None, use_bias=True)
                            # self.conv_middle = GATConv(nPrd, attn_heads=head, concat_heads=True,
                            #                            add_self_loops=True, activation=None, use_bias=True)
                            # self.conv2 = GATConv(nPrd, attn_heads=head, concat_heads=True,
                            #                      add_self_loops=True, activation=PReLU(), use_bias=True)
                        elif GNN == "GCNConv":
                            # # GCNConv
                            ...
                            self.conv2 = DiffusionConv(nPrd, K=6, activation=PReLU(), kernel_initializer='glorot_uniform', kernel_regularizer=None, kernel_constraint=None)
                        elif GNN == "GCSConv":
                            # GCSConv
                            ...
                            # x: (B, nBus, nPrd)  -> (B, nBus, nPrd)
                            self.conv1 = Dense(nPrd, activation=None)
                            self.conv_middle = Dense(nPrd, activation=None)
                            self.conv2 = Dense(nPrd, activation=None)
                            self.gnn_prelu = PReLU()
                        ################################################################################################################################################################
                        if RNN == "GRU":
                            ...
                            if nPrd == 8:
                                self.lstm1 = TCN(nb_filters=nBus, kernel_size=2, dilations=(1, 2, 4), padding='causal',
                                                 activation=PReLU(), return_sequences=True, dropout_rate=0.3, kernel_initializer='glorot_uniform',use_batch_norm=True)
                            elif nPrd == 24:
                                self.lstm1 = TCN(nb_filters=nBus, kernel_size=3, dilations=(1, 3, 9), padding='causal',
                                                 activation=PReLU(), return_sequences=True, dropout_rate=0.3, kernel_initializer='glorot_uniform',use_batch_norm=True)

                        elif RNN == "DNN":
                            # x를 (B, nPrd, nBus)로 바꾼 뒤 Dense 1층
                            self.lstm1 = Dense(nBus, activation=None)
                            self.rnn_prelu = PReLU()


                            # if nPrd == 8:
                            #     self.lstm1 = TCN(nb_filters=nBus, kernel_size=2, dilations=(1, 2, 4), padding='causal',
                            #                      activation=PReLU(), return_sequences=True, kernel_initializer='glorot_uniform')
                            # elif nPrd == 24:
                            #     self.lstm1 = TCN(nb_filters=nBus, kernel_size=3, dilations=(1, 3, 9), padding='causal',
                            #                      activation=PReLU(), return_sequences=True, kernel_initializer='glorot_uniform')
                        # self.drop = Dropout(0.05)

                        # self.dense_inter1 = Dense(nPrd, activation="relu")
                        # self.dense_inter2 = Dense(nPrd, activation="relu")


                        self.dense = Dense(nPrd*2, activation="sigmoid")

                        self.dense1 = Dense(nBus, activation="relu") # , activation="relu"
                        self.dense_middle = Dense(nBus*1, activation="relu")
                        self.dense2_middle = Dense(nBus*1, activation="relu")
                    ...
                    def call(self, inputs, labels):
                        if GNN == "ECCConv": # edge 사용하는 layer
                            ...
                            # x = self.dense_inter1(x)
                            # x = self.dense_inter2(x)
                        ...
                        if RNN in  ["GRU", "LSTM"]: # , "TCN"
                            ...
                            x = tf.transpose(x, [0, 2, 1])  # (B, nPrd, nBus)
                            out = self.rnn_prelu(self.lstm1(x))  # Dense 1층 + PReLU

                            output = self.dense1(out)  # (B, nPrd, nBus)
                            out = tf.transpose(output, [0, 2, 1])  # (B, nBus, nPrd)
                            out = self.dense_middle(out)
                            out = self.dense2_middle(out)
                            output = self.dense2(out)  # (B, nBus, nPrd*2)
                            output = tf.reshape(output, [tf.shape(output)[0], nBus, nPrd, 2])
                            return output, labels
                ...
                results = []
                trackGNNLSTM = np.zeros([epochs, 12])
                for batch in loader_trGNNLSTM:
                    step += 1

                    with tf.GradientTape() as tape:
                        prediction, target = modelGNNLSTM(*batch, training=True)
                        NMAE_total, NMAE_m, NMAE_a, mae_m, mae_A = NMAE(target, prediction)

                        mae = mae_m*0.5 + mae_A*0.5

                        loss = loss_fn(target, prediction)# change loss
                    ...
                    results.append((loss.numpy(), mae.numpy(), smape_total.numpy(), smape_m.numpy(), smape_A.numpy(),
                                    NMAE_total.numpy(), NMAE_m.numpy(), NMAE_a.numpy()))
                    ...
                    if step == loader_trGNNLSTM.steps_per_epoch:
                        ...
                        loader_vaGNNLSTM = MixedLoader(data_vaGNNLSTM, batch_size=batch_sizes, shuffle=False, epochs=1)
                        val_loss, val_mae, val_smape_total, val_smape_m, val_smape_A, val_nmae, val_nmae_m, val_nmae_A, val_mae_m, val_mae_A = evaluateGNNLSTM(
                            loader_vaGNNLSTM)
                        ...
                        val_mae_m_np = val_mae_m.numpy() if isinstance(val_mae_m, tf.Tensor) else val_mae_m
                        val_mae_A_np = val_mae_A.numpy() if isinstance(val_mae_A, tf.Tensor) else val_mae_A
                        val_smape_total_np = val_smape_total.numpy() if isinstance(val_smape_total,
                                                                                   tf.Tensor) else val_smape_total
                        ...
                        val_nmae_np = val_nmae.numpy()
                        val_nmae_m_np = val_nmae_m.numpy()
                        val_nmae_A_np = val_nmae_A.numpy()
                        ...
                        print("Ep. {} - loss: {:.5f} - MAE: {:.5f} - SMAPE: {:.5f} (m: {:.5f}, A: {:.5f}) - "
                              "NMAE: {:.5f} (m: {:.5f}, A: {:.5f}) - "
                              "Val mse: {:.5f} - Val MAE: {:.5f} (m: {:.5f}, A: {:.5f}) - Val SMAPE: {:.5f} (m: {:.5f}, A: {:.5f}) - "
                              "Val NMAE: {:.5f} (m: {:.5f}, A: {:.5f})".format(
                            epoch,
                            np.mean(results, 0)[0],  # loss
                            np.mean(results, 0)[1],  # MAE
                            np.mean(results, 0)[2],  # SMAPE_total
                            np.mean(results, 0)[3],  # SMAPE_m
                            np.mean(results, 0)[4],  # SMAPE_A
                            np.mean(results, 0)[5],  # Train NMAE_total
                            np.mean(results, 0)[6],  # Train NMAE_m
                            np.mean(results, 0)[7],  # Train NMAE_A
                            val_loss_np,  # Validation mse
                            val_mae_np,  # Validation MAE
                            val_mae_m_np,  # Validation MAE_m
                            val_mae_A_np,  # Validation MAE_A
                            val_smape_total_np,  # Validation SMAPE_total
                            val_smape_m_np,  # Validation SMAPE_m
                            val_smape_A_np,  # Validation SMAPE_A
                            val_nmae_np,  # Validation NMAE
                            val_nmae_m_np,  # Validation NMAE_m
                            val_nmae_A_np))  # Validation NMAE_A
                ...
                # ✅ 학습이 완료된 후, trackGNNLSTM에 기록된 데이터에서 loss 추출
                epochs_ran = np.arange(epoch)  # 실제 학습된 epoch 수만큼

                train_loss_curve = trackGNNLSTM[:epoch, 0]  # train loss (첫 번째 열)
                val_loss_curve = trackGNNLSTM[:epoch, 8]  # validation loss (9번째 열: val_loss_np)

                # ✅ 시각화
                plt.figure(figsize=(7, 5))

                plt.plot(epochs_ran, train_loss_curve, label='Training Loss', linewidth=2, color='#4C72B0')
                plt.plot(epochs_ran, val_loss_curve, label='Validation Loss', linewidth=2, color='#C44E52')
                ...
                # ✅ 저장 경로 설정
                loss_curve_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/result/{GNN}_{RNN}_loss_curve.png"
                plt.savefig(loss_curve_path, dpi=400, bbox_inches='tight')
                plt.show()
                ...
                modelGNNLSTM.compile(optimizer=optimizer, loss=NMAE, metrics=[NMAE]) # loss=loss_fn,

                #save_path = DataDir + f"/{nBus}bus/{nPrd}/GNN_trained_model/Spatio-Temporal_NodePrediction_" + SysName + "_SavedModel"
                save_path_MA = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/GNN_trained_model/" + GNN +"_"+ RNN + "_modelMA_SavedModel"
                ...
                test_loss, test_mae, test_smape, test_smape_m, test_smape_A, test_nmae, test_nmae_m, test_nmae_A, test_mae_m, test_mae_A = evaluateGNNLSTM(
                    loader_teGNNLSTM)
                ...
                Test MAE_M: {test_mae_m:.5f}
                Test MAE_A: {test_mae_A:.5f}
                Test SMAPE: {test_smape:.5f}
                Test SMAPE_M: {test_smape_m:.5f}
                Test SMAPE_A: {test_smape_A:.5f}
                Test NMAE: {test_nmae:.5f}
                Test NMAE_M: {test_nmae_m:.5f}
                Test NMAE_A: {test_nmae_A:.5f}
                Total Test Time: {elapsed_time:.5f} 
                Time per dataset: {time_per_dataset:.5f} 
                """

                # 파일로 저장
                output_file = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/result/{GNN}_{RNN}_results.txt"
                with open(output_file, "w") as f:
                    f.write(output_text)

                print(f"Results saved to {output_file}")
                print(f" Test MAE: {test_mae:.5f}")
                print(f" Test MAE_M: {test_mae_m:.5f}")
                print(f" Test MAE_A: {test_mae_A:.5f}")
                print(f" Test SMAPE: {test_smape:.5f}")
                print(f" Test SMAPE_M: {test_smape_m:.5f}")
                print(f" Test SMAPE_A: {test_smape_A:.5f}")
                print(f" Test NMAE: {test_nmae:.5f}")
                print(f" Test NMAE_M: {test_nmae_m:.5f}")
                print(f" Test NMAE_A: {test_nmae_A:.5f}")

            if mode == 'save_output':

                # model_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/GNN_trained_model/sigx_GATConv_TCN_modelMA_SavedModel"
                model_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/8/GNN_trained_model/!_GATConv_TCN_modelMA_SavedModel"
                # output_dir = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/sigx_failed_predictions"
                output_dir = f"{PROJECT_ROOT}/Data/25_1017 access review/1354bus/8"
                ...
                Demand_fileName = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/demand1354Bus_{nPrd}_failed_all.txt'
                dfNDmd = loadtxt(Demand_fileName, delimiter=',')  # Nodal demand NBus*24-Hours sequences

                # 노드의 M A데이터셋
                madata = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/1354_{nPrd}_ma_values_failed_all.txt'
                dfLFlw = loadtxt(madata, delimiter=',')
                # each graph(sample) must have 24 Nodal values and edge connection(constant, topology does not chage)
                x_data = dfNDmd
                nSamples = len(x_data)
                ...
                path = f'{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/failed_graph_data'
                if not os.path.exists(path):
                    os.makedirs(path)
            ...
            if mode == 'save_excel':
                ...
                model_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/24/GNN_trained_model/DNN1_decay95_GATConv_TCN_modelMA_SavedModel"
                output_dir = f"{PROJECT_ROOT}/Data/same proportions/1354bus/24/test_excel/NMAE_m5a5_test_excel/"
            ...
            if mode == 'test':
                ...
                # model_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/GNN_trained_model/sigx_GATConv_TCN_modelMA_SavedModel"
                model_path = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/GNN_trained_model/!_GATConv_TCN_modelMA_SavedModel"
                # output_dir = f"{PROJECT_ROOT}/Data/same proportions/1354bus/{nPrd}/sigx_failed_predictions"
                output_dir = f"{PROJECT_ROOT}/Data/25_1017 access review/1354bus/{nPrd}"
                ...
                # 파일로 저장
                output_file = f"{PROJECT_ROOT}/Data/25_1017 access review/1354bus/{nPrd}/{GNN}_{RNN}_results_all.txt"
                with open(output_file, "w") as f:
